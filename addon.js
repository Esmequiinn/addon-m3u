...
// Load & parse the M3U list
let movies = [];
let series = {};

// Descargar M3U desde URL remota
...
async function loadList() {
  if (!M3U_URL) {
    console.warn("⚠️ No se definió M3U_URL");
    return;
  }
  ...
  try {
    ...
    console.log(`✅ Lista cargada: ${movies.length} películas, ${Object.keys(series).length} series`);
...
function movieToFullMeta(m) {
  return {
    ...movieToMeta(m),
    description: m.description || "Película de tu lista M3U personal.",
    year: m.year || null,
  };
}
...
function seriesToFullMeta(s, id) {
  ...
  return {
    ...
    name: s.title,
    poster: s.poster || makePosterUrl(s.title),
    description: s.description || "Serie de tu lista M3U personal.",
    videos: Object.entries(seasons).flatMap(([, eps]) => eps),
  };
}
